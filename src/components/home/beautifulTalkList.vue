<template>
  <div class="beautifulTalkList" :style="bgCss">
    ...
    <div class="goodsListContainer">
        <ul class="new_box">
            <li class="newItem clearfix" v-for="item in beautifulTalkList" :key="item.fashionId">
                <div class="detail">
                    <h4>{{item.title}}</h4>
...
export default {
    ...
    created(){
        this.$axios.get("/dbapi/index/data.action?").then(res=>{
            this.beautifulTalkList=(res.data.response.resultList.beautifulTalkList).slice(0,3);
        });
    }
}
